...
(function(root, factory){
  ...
  function generateDocx(exam, opts){
    ...
    if (templateHdr){
      const W = [3300, 3000, 3338];
      const hrows = [];
      hrows.push(trX([
        tcX(runX(exam.university || 'University', { b:true, sz:26 }), { w:9638, span:3, ctr:true })
      ], W));
      hrows.push(trX([
        tcX(runX('Level: ' + (exam.level || ''), { b:true }), { w:3300 }),
        tcX(runX('', {}), { w:3000 }),
        tcX(runX('Semester: ' + (exam.semester || ''), { b:true }), { w:3338 })
      ], W));
      hrows.push(trX([
        tcX(runX(exam.faculty || exam.dept || '', { b:true }), { w:6300, span:2 }),
        tcX(runX(exam.type || '', { b:true, ctr:true }), { w:3338, ctr:true })
      ], W));
      hrows.push(trX([
        tcX(runX('Program: ' + (exam.program || ''), { b:true }), { w:3300 }),
        tcX(runX('', {}), { w:3000 }),
        tcX(runX('Date: ' + (exam.date ? longDate(exam.date) : ''), { b:true }), { w:3338 })
      ], W));
      hrows.push(trX([
        tcX(runX('Course Name: ' + (exam.course || ''), { b:true }), { w:3300 }),
        tcX(runX('Code: ' + (exam.code || ''), { b:true }), { w:3000 }),
        tcX(runX('Time allowed: ' + (exam.time || ''), { b:true }), { w:3338 })
      ], W));
      ...
      const tline = [exam.type, exam.date ? 'Date: ' + longDate(exam.date) : '', exam.time ? 'Time allowed: ' + exam.time : (exam.duration ? 'Duration: ' + exam.duration + ' minutes' : ''), exam.totalMarks ? 'Total Marks: ' + exam.totalMarks : ''].filter(Boolean);
