...
(function(){
  ...
  function buildPaperHTML(examObj, opts){
    ...
    if (templateHdr){
      const t = headerTableHTML(examObj);
      e.push(t.replace('<table class="hdr-prev-table">', '<table class="hdr-prev-table paper-tbl">'));
    } else {
      ...
      const tm = [examObj.type, examObj.model ? 'Model ' + esc(examObj.model) : '', examObj.date ? 'Date: '+esc(longDate(examObj.date)) : '', examObj.time ? 'Time allowed: '+esc(examObj.time) : (examObj.duration ? 'Duration: '+examObj.duration+' min' : ''), examObj.totalMarks ? 'Total Marks: '+examObj.totalMarks : ''].filter(Boolean);
    ...
    try {
      ...
      try {
        ...
        if (tpl){
          ...
          const img1 = zip.file('word/media/image1.png');
          const img2 = zip.file('word/media/image2.png');
          if (img1) logos.img1 = img1.asUint8Array();
          if (img2) logos.img2 = img2.asUint8Array();
        }
      } catch (e){ /* header renders without logos */ }
      const blob = TaskyDocx.generateDocx(exam, { teacher, logos });
  ...
  function initCatalog(){
    const depts = CATALOG.departments.slice();
    if (depts.indexOf('Faculty of Applied Health Science') === -1) depts.unshift('Faculty of Applied Health Science');
    if ($('#dl-dept'))    $('#dl-dept').innerHTML    = depts.map(v => '<option value="' + esc(v) + '">').join('');
  ...
  function latexFromExam(exam){
    ...
    const isFinal = String(exam.type || '').toLowerCase().indexOf('final') !== -1;
    ...
    lines.push(row(exam.faculty || 'Faculty') + ' & ' + row(title + (exam.model ? ' · Model ' + exam.model : '')) + ' &  \\\\ \\hline');
  ...
  function wordTemplateForExam(exam, genOpts){
    ...
    const vals = {
      ...
      type   : (exam.type || '').trim() + (exam.model ? '  ·  Model ' + String(exam.model).trim() : ''),
...
{ key:'type',   match:/^final/i,          mode:'replace' },
    ...
    /* final exams are anonymous — strip the "Student name: … / Student ID: …"
       paragraph (and its underline divider) baked into the header form */
    const isFinal = String(exam.type || '').toLowerCase().indexOf('final') !== -1;
    if (isFinal){
      const idRe = /student\s*(name|id)|name\s*:.*id\s*:/i;
      const undRe = /^_{3,}\s*$/;
      const idBlocks = [];
      let bm;
      bodyRe.lastIndex = 0;
      while ((bm = bodyRe.exec(updated)) !== null) idBlocks.push({ start: bm.index, end: bm.index + bm[0].length, html: bm[0] });
      const drops = [];
      idBlocks.forEach((b, i)=>{
        const t = plain(b.html).trim();
        if (idRe.test(t)){
          drops.push(b);
          const next = idBlocks[i + 1];
          if (next && undRe.test(plain(next.html).trim())) drops.push(next);
        }
      });
      drops.sort((a, b)=> b.start - a.start);
      drops.forEach(b=>{ updated = updated.slice(0, b.start) + updated.slice(b.end); });
    }
  ...
  function initHeaderLogo(){
    try {
      ...
      headerLogo = toUrl(zip.file('word/media/image1.png'));
      headerLogo2 = toUrl(zip.file('word/media/image2.png'));
    } catch (e){ /* preview just renders without the logos */ }
  }
  ...
  function renderHeaderPreview(){
    ...
    wrap.innerHTML = headerTableHTML(exam);
  }

  /* Render the official header the same style as the bundled 'temp exam .docx' —
     tall logo row, Calibri labels and the underlined Albertus title for the exam type. */
  function headerTableHTML(exam){
    const logo1 = headerLogo ? '<img class="hp-logo" src="' + headerLogo + '" alt="">' : '';
    const logo2 = headerLogo2 ? '<img class="hp-logo-sm" src="' + headerLogo2 + '" alt="">' : '';
    const title = (exam.type || '') + (exam.model ? ' &nbsp;·&nbsp; Model ' + esc(exam.model) : '');
    /* column proportions of the official header form (grid: 3120/1275/1845/4268) */
    const c0 = '29.7%', mid = '29.7%', c3 = '40.6%', half1 = '41.8%', half2 = '58.2%';
    return '<table class="hdr-prev-table">' +
      '<tr>' +
        '<td colspan="2" class="hp-univ hp-top" style="width:' + half1 + '">' + logo1 + esc(exam.university || 'University') + '</td>' +
        '<td colspan="2" class="hp-fac hp-top" style="width:' + half2 + '">' + logo2 + esc(exam.faculty || exam.dept || '') + '</td>' +
      '</tr>' +
      '<tr>' +
        '<td class="hp-label" style="width:' + c0 + '">Level: ' + esc(exam.level || '') + '</td>' +
        '<td colspan="2" class="hp-type" style="width:' + mid + '">' + title + '</td>' +
        '<td class="hp-label" style="width:' + c3 + '">Semester: ' + esc(exam.semester || '') + '</td>' +
      '</tr>' +
      '<tr>' +
        '<td class="hp-label" style="width:' + c0 + '">Program: ' + esc(exam.program || '') + '</td>' +
        '<td colspan="2" style="width:' + mid + '"></td>' +
        '<td class="hp-label" style="width:' + c3 + '">Date: ' + esc(longDate(exam.date)) + '</td>' +
      '</tr>' +
      '<tr>' +
        '<td class="hp-label" style="width:' + c0 + '">Course: ' + esc(exam.course || '') + '</td>' +
        '<td colspan="2" class="hp-label" style="width:' + mid + '">Code: ' + esc(exam.code || '') + '</td>' +
        '<td style="width:' + c3 + '"></td>' +
      '</tr>' +
      '</table>';
  }
  ...
  function downloadHeaderOnly(){
    ...
    try {
      ...
      const blob = wordTemplateForExam(headerOnly);
  ...
  $('#exam-type').addEventListener('change', ()=>{ updateSummary(); renderHeaderPreview(); });
  ...
  function init(){
    ...
    $('#exam-faculty').value = 'Faculty of Applied Health Science';
    $('#exam-time').value = '2 hours';
    applySemesterFromDate();
